...
import httpx
import urllib.parse
from utility.field_extractors import UserFieldExtractor, detect_list_response_mode, get_swagger_schema_for_discovery
from utils import (
    is_valid_uuid,
    ZEDEDA_API_BASE,
    USER_AGENT,
...
def register_user_tools(mcp):
    """Register all user-related MCP tools (GET methods only)."""
    USE_MOCK_API_MCP_DATA = getattr(mcp, "USE_MOCK_API_MCP_DATA", False)
    logger.info(
        f"[TOOL] User tools registered with USE_MOCK_API_MCP_DATA={USE_MOCK_API_MCP_DATA}"
    )

    @mcp.tool(tags={"iam", "get_all_users", "users", "authentication"})
    async def get_all_users(
        ...
        """
        Query users from Zedcloud with comprehensive filtering options.

        This tool provides flexible querying of user information with support
        ...
        Args:
            ...
            name_pattern: Filter by user name pattern (supports wildcards)
            all: Include all users regardless of permissions (boolean)
        ...
        Returns:
            Dictionary containing list of users with metadata including IDs, names,
            email addresses, and role information. Returns error message on failure.
        """
        # Use the pre-configured UserFieldExtractor
        field_extractor = UserFieldExtractor()
        ...
        if USE_MOCK_API_MCP_DATA:
            mock = load_mock_json("users-list.json", required=USE_MOCK_API_MCP_DATA)
            if mock is not None:
                logger.info("[MOCK] Returning mock data for users query")
        ...
        logger.debug("Invoked query_zededa_users tool")
        ...
        url = f"{ZEDEDA_API_BASE}/api/v1/users"
        ...
        logger.info(f"Querying users from URL: {url}")
    ...
    @mcp.tool(tags={"iam", "user", "authentication", "lookup by id or name"})
    async def get_user(
        ...
        Use this tool when you need to get complete configuration and metadata for a user.
        The tool automatically resolves the user from either its unique ID or human-readable name.

        Args:
            identifier: The user ID or name to look up
            lookup_by: Search method - "id" for user ID lookup or "name" for name lookup (default: "name")

        Returns:
            Dictionary containing user details including ID, name, email, role assignments,
            and authentication configuration. Returns error message if user not found.
        """
        if USE_MOCK_API_MCP_DATA:
            mock = load_mock_json("users-detail.json")
            if mock is not None:
                logger.info(
                    f"[MOCK] Returning mock data for user {lookup_by}='{identifier}'"
                )
                ...
                if filtered_mock is None:
                    return f"User not found. Check that the {lookup_by} '{identifier}' is correct."
        ...
        if lookup_by == "id" and not is_valid_uuid(identifier):
            ...
            url = f"{ZEDEDA_API_BASE}/api/v1/users/name/{urllib.parse.quote(identifier, safe='')}"
            headers = {
                "User-Agent": USER_AGENT,
                "Accept": "application/json",
                "Authorization": token,
            ...
            async with httpx.AsyncClient() as client:
                try:
                    ...
                    result["_lookup_note"] = (
                        f"Note: '{identifier}' was provided as an ID but is not a valid UUID. Successfully found user by name instead. For future requests, use lookup_by='name' for this user."
                    )
                    return result
                except Exception as e:
                    logger.error(
                    ...
                    return f"Invalid user ID format (not a UUID): '{identifier}'. Also attempted name-based lookup but user not found. Please verify the identifier."

        # Build URL based on lookup method
        lookup_endpoint = "id" if lookup_by == "id" else "name"
        encoded_identifier = urllib.parse.quote(identifier, safe="")
        url = f"{ZEDEDA_API_BASE}/api/v1/users/{lookup_endpoint}/{encoded_identifier}"
        ...
        logger.info(f"Retrieving user {lookup_by}='{identifier}'")
    ...
    @mcp.tool(tags={"iam", "self_user", "authentication", "current user"})
    async def get_user_self() -> dict[str, Any] | str:
        """
        Get the current user's own user record.

        This tool retrieves the user information for the authenticated user,
        useful for understanding the current context, permissions, and profile.

        Returns:
            Dictionary containing the current user's details including ID, name,
            email, role assignments, and authentication configuration.
        """
        if USE_MOCK_API_MCP_DATA:
            mock = load_mock_json("user-self.json", required=USE_MOCK_API_MCP_DATA)
            if mock is not None:
                logger.info("[MOCK] Returning mock data for user self")
                return mock
        ...
        url = f"{ZEDEDA_API_BASE}/api/v1/users/self"
        ...
        logger.info("Retrieving current user's information")
        ...
        async with httpx.AsyncClient() as client:
            try:
                ...
                if e.response.status_code == 404:
                    return "No user found for current authentication."
                elif e.response.status_code == 403:
                    return "Access denied: Unable to retrieve own user information."
                return f"HTTP error {e.response.status_code}: {e.response.text}"
            except Exception as e:
                logger.error(f"Unexpected error - GET {url}: {e}")
                return f"Unexpected error: {e}"
